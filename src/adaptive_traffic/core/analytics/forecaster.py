...
class TrafficForecaster:
    ...
    def update(self, timestamp: datetime, flow_data: Dict[str, float], 
               speed_data: Optional[Dict[str, float]] = None):
    ...
    def forecast(self, current_time: datetime) -> TrafficForecast:
        ...
        for direction in self.flow_history:
            ...
            flow_pred, flow_ci = self._forecast_series(
                self.flow_history[direction], current_time
            )
            ...
            if direction in self.speed_history:
                speed_pred, _ = self._forecast_series(
                    self.speed_history[direction], current_time
                )
            ...
            congestion_preds[direction] = [
                min(1.0, f / capacity) for f in flow_pred
            ]
    ...
    def _forecast_series(self, history: List[float], 
                        current_time: datetime) -> Tuple[List[float], Tuple[List[float], List[float]]]:
        ...
        for direction, flow in flows.items():
            ...
            if recent_std > 0:
                ...
                if z_score > self.anomaly_threshold:
                    logger.warning(f"Anomaly detected: {direction} flow={flow}, "
