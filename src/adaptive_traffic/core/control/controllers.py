...
class BaseController(ABC):
    ...
    def __init__(self, config: dict, city_profile: Optional[CityProfile] = None):
        ...
        # Use city profile if available, otherwise fall back to config
        if city_profile:
            self.min_green = city_profile.signal_bounds.min_green
            self.max_green = city_profile.signal_bounds.max_green
            self.yellow_time = city_profile.signal_bounds.yellow
            self.all_red_time = city_profile.signal_bounds.all_red
            self.directions = city_profile.intersection_geometry.approaches
        else:
...
class FuzzyController(BaseController):
    ...
    def compute_timing(self, state: TrafficState) -> SignalTiming:
        ...
        cycle = sum(green_times.values()) + len(self.directions) * (
            self.yellow_time + self.all_red_time
        )
...
def create_controller(
    controller_type: str, config: dict, city_profile: Optional[CityProfile] = None
) -> BaseController:
    """Factory function to create signal controller

    Args:
        controller_type: Type of controller (fixed, webster, fuzzy, dqn)
        config: Controller configuration
        city_profile: Optional city profile for city-specific parameters
    """
    controllers = {
    ...
    return controller_class(config, city_profile)
