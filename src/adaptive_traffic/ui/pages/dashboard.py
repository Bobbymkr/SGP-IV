...
def show_traffic_metrics():
    ...
    capacity_lines = alt.Chart(df).mark_rule(
    ...
    pattern_data = pd.DataFrame({
        ...
        'Volume': np.concatenate([
            150 + 100 * np.sin((np.array(hours) - 8) * np.pi / 12) + np.random.normal(0, 10, 24),
            120 + 80 * np.sin((np.array(hours) - 9) * np.pi / 12) + np.random.normal(0, 8, 24),
            100 + 60 * np.sin((np.array(hours) - 7) * np.pi / 12) + np.random.normal(0, 6, 24),
            80 + 40 * np.sin((np.array(hours) - 10) * np.pi / 12) + np.random.normal(0, 5, 24)
        ])
    })

    pattern_chart = alt.Chart(pattern_data).mark_line().encode(
...
def show_performance():
    ...
    comp_chart = alt.Chart(comp_df).mark_bar().encode(
        x='System:N',
        y='Value:Q',
        color=alt.Color('System:N', scale=alt.Scale(domain=['Before', 'After'], range=['#dc3545', '#28a745'])),
        column='Metric:N',
    ...
    eff_chart = alt.Chart(eff_df).mark_line(point=True).encode(
        x='Date:T', y=alt.Y('Efficiency (%):Q', scale=alt.Scale(domain=[70, 100])),
...
def show_alerts():
    ...
    alerts = [
        {"time": "08:32", "type": "🚦 Traffic", "severity": "High", "msg": "Unusual congestion at Main St & 5th Ave", "status": "Active"},
        {"time": "07:45", "type": "💻 System", "severity": "Medium", "msg": "Camera #3 intermittent connectivity", "status": "Active"},
        {"time": "23:12", "type": "🔧 Maintenance", "severity": "Low", "msg": "Scheduled update completed", "status": "Resolved"},
        {"time": "17:30", "type": "🔒 Security", "severity": "Critical", "msg": "Unauthorized access attempt", "status": "Resolved"},
    ]

    for alert in alerts:
        severity_color = {"Critical": "#dc3545", "High": "#fd7e14", "Medium": "#ffc107", "Low": "#6c757d"}[alert["severity"]]
