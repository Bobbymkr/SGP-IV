...
class CameraConfig(BaseModel):
    ...
    intersection_id: str
    direction: str
    roi_polygons: dict[str, list[list[int]]] | None = None
    enabled: bool = True
...
async def detect_batch(files: list[UploadFile] = File(...), camera_id: str = Form(...)):
...
async def get_detection_history(camera_id: str | None = None, limit: int = 100, offset: int = 0):
...
async def get_vehicle_counts(camera_id: str, window_minutes: int = 5):
    ...
    return {
        ...
        "counts_by_class": {"car": 142, "truck": 23, "bus": 8, "motorcycle": 15, "bicycle": 31},
        "counts_by_direction": {
