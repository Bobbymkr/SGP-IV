    ...
    try:
        ...
        return {
            ...
            "confidence_intervals": {
                k: {"lower": v[0], "upper": v[1]} for k, v in forecast.confidence_intervals.items()
            },
        }
...
async def evaluate_scenario(request: ScenarioRequest):
    ...
    scenario = {"flow_modifiers": request.flow_modifiers, "duration_hours": request.duration_hours}
...
async def get_metrics(intersection_id: str, window_hours: int = Query(1, ge=1, le=168)):
    ...
    return {
        ...
        "avg_flow": {"north": 450, "south": 420, "east": 380, "west": 350},
        "avg_speed": {"north": 42.5, "south": 44.2, "east": 38.7, "west": 35.1},
        "congestion_level": {"north": 0.65, "south": 0.58, "east": 0.72, "west": 0.78},
        "total_vehicles": 6542,
        "avg_wait_time": 28.5,
...
async def get_performance(intersection_id: str, days: int = Query(7, ge=1, le=90)):
...
async def get_environmental_impact(intersection_id: str):
    ...
    return {
        ...
        "emissions_breakdown": {"CO2": 124.0, "NOx": 2.8, "PM2_5": 0.45, "VOCs": 1.2, "CO": 12.5},
