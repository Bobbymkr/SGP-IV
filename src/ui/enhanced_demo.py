...
if page == "🏠 System Overview":
    st.markdown(
        "<h1 class='main-header'>Intelligent Traffic Signal Control</h1>", unsafe_allow_html=True
    )
    ...
    with col2:
        ...
        try:
            ...
            st.image(
                "Demo.gif", caption="Adaptive Traffic Signal System Demo", use_column_width=True
            )
        except:
    ...
    st.markdown(
        "<h1 class='main-header'>Live Interactive Traffic Demo</h1>", unsafe_allow_html=True
    )
    ...
    with col1:
        ...
        north_traffic = st.slider(
            "⬆️ Northbound Traffic", 0, 100, 30, help="Vehicles per minute approaching from north"
        )
        south_traffic = st.slider(
            "⬇️ Southbound Traffic", 0, 100, 25, help="Vehicles per minute approaching from south"
        )

    with col2:
        st.markdown("#### East-West Roads")
        east_traffic = st.slider(
            "➡️ Eastbound Traffic", 0, 100, 45, help="Vehicles per minute approaching from east"
        )
        west_traffic = st.slider(
            "⬅️ Westbound Traffic", 0, 100, 35, help="Vehicles per minute approaching from west"
        )
    ...
    try:
        st.image(
            "Demo.gif", caption="Adaptive Traffic Signal System in Operation", use_column_width=True
        )
        st.markdown(
            """
        <div class="explanation-box">
    ...
    st.markdown(
        "<h1 class='main-header'>Algorithm Performance Comparison</h1>", unsafe_allow_html=True
    )
    ...
    efficiency_chart = (
        ...
        .encode(
            ...
            y=alt.Y(
                "Efficiency:Q", title="Traffic Efficiency (%)", scale=alt.Scale(domain=[80, 150])
            ),
            color=alt.Color("Algorithm:N", legend=None),
            tooltip=["Algorithm:N", "Efficiency:Q"],
        )
        .properties(title="Traffic Efficiency Comparison", height=300)
    )
    ...
    wait_chart = (
        ...
        .encode(
            x=alt.X("Algorithm:N", title="Algorithm"),
            y=alt.Y(
                "Wait Time:Q", title="Relative Wait Time (%)", scale=alt.Scale(domain=[40, 110])
            ),
            color=alt.Color("Algorithm:N", legend=None),
            tooltip=["Algorithm:N", "Wait Time:Q"],
        )
        .properties(title="Relative Wait Time Comparison", height=300)
    )
    ...
    try:
        st.image(
            "Demo.gif",
            caption="Adaptive System vs. Traditional Fixed Timing",
    ...
    impact_chart = (
        ...
        .properties(title="Cumulative Environmental Impact", height=400)
    )
    ...
    emergency_type = st.selectbox(
        "Select Emergency Vehicle Type", ["Ambulance 🚑", "Fire Truck 🚒", "Police 🚓"]
    )

    approach_direction = st.selectbox("Approach Direction", ["North", "South", "East", "West"])
