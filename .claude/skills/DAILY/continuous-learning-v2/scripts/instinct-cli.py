...
def detect_project() -> dict:
    ...
    hash_source = (
        normalized_remote if normalized_remote else (remote_url if remote_url else fallback_root)
    )
...
def _merge_instinct_dir(from_dir: Path, into_dir: Path, existing_ids: set[str]) -> tuple[int, int]:
    ...
    for file_path in sorted(from_dir.iterdir()):
        ...
        if target_path.exists():
            target_path = (
                into_dir / f"{file_path.stem}-{_project_hash(str(file_path))}{file_path.suffix}"
            )
