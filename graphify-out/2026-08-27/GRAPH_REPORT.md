...
## Corpus Check
- 72 files · ~39,658 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1026 nodes · 1340 edges · 99 communities (67 shown, 32 thin omitted)
...
5. `VehicleDetection` - 15 edges
6. `FuzzyController` - 14 edges
7. `🚦 Adaptive Traffic Signal Timer` - 14 edges
8. `OnnxDetector` - 13 edges
9. `UltralyticsDetector` - 13 edges
10. `Intersection` - 13 edges
...
Cohesion: 0.08
Nodes (32): OnnxDetector, DetectionResult, ndarray, ONNX Detection Adapter CPU/NPU backend — loads models from…, ONNX Runtime vehicle detector (CPU/NPU providers), create_detector(), MultiCameraDetector, ndarray (+24 more)

### Community 2 - "FuzzyController"
Cohesion: 0.07
Nodes (28): BaseController, ControllerType, create_controller(), DQNController, FixedTimeController, FuzzyController, ABC, Enum (+20 more)
...
Cohesion: 0.38
Nodes (9): Path, check(), _class_index(), convert_coco(), convert_yolo(), main(), make_calibration(), Dataset normalizer/validator — enforces docs/DATASET_SPEC.md contract. Usage:… (+1 more)
...
Cohesion: 0.25
Nodes (7): create_digital_twin(), create_forecaster(), create_predictive_analytics(), Predictive Analytics Module, Factory function to create forecaster, Factory function to create digital twin, Factory function to create predictive analytics engine
...
## Knowledge Gaps
- **165 isolated node(s):** `adaptive-traffic-signal`, `graphify`, `Verification (single source of truth)`, `Dependency rule`, `graphify` (+160 more)
...
- **Are the 5 inferred relationships involving `VehicleDetection` (e.g. with `OnnxDetector` and `MultiCameraDetector`) actually correct?**
  _`VehicleDetection` has 5 INFERRED edges - model-reasoned connections that need verification._
...
- **Should `VehicleDetection` be split into smaller, more focused modules?**
