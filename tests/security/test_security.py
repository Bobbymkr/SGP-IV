            ...
            with self.subTest(filename=filename):
                ...
                with patch('cv2.imread', return_value=None):
                    with patch('builtins.print') as mock_print:
                        ...
                        error_detected = any('error' in call.lower() or 'not found' in call.lower() 
                                          for call in print_calls)
                        self.assertTrue(error_detected or len(print_calls) > 0, 
                ...
                with patch('os.listdir', return_value=[filename]):
                    with patch('builtins.print') as mock_print:
                        ...
                        self.assertEqual(len(filtered_files), 0, 
                                       f"Malicious file {filename} not filtered out")
        ...
        for malicious_input in malicious_inputs:
            with self.subTest(input=malicious_input):
                ...
                with patch('subprocess.run') as mock_run:
                    ...
                    try:
                        result = subprocess.run(['python', 'vehicle_detection_modern.py', malicious_input], 
                    ...
                    if mock_run.called:
                        ...
                        dangerous_patterns = ['rm -rf', 'cat /etc/', 'nc ', 'curl ', '&&', ';', '`', '$(']
                        
                        for pattern in dangerous_patterns:
                            if pattern in command_str:
                                self.fail(f"Potential command injection detected: {pattern} in {command_str}")
...
class TestFileSystemSecurity(unittest.TestCase):
    ...
    def test_01_directory_traversal_prevention(self):
        ...
        for payload in traversal_payloads:
            with self.subTest(payload=payload):
                ...
                if '..' in unsafe_path:
                    self.assertNotEqual(unsafe_path, safe_path, 
                                       "Directory traversal not properly prevented")
    ...
    def test_02_file_permission_validation(self):
        ...
        test_scenarios = [
            ...
            {'file': 'C:\\Windows\\System32\\config\\SAM', 'should_exist': False, 'reason': 'Windows system file'},
        ...
        for scenario in test_scenarios:
            with self.subTest(file=scenario['file']):
                ...
                if not scenario['should_exist']:
                    ...
                    if exists:
                        self.fail(f"Sensitive file accessible: {scenario['file']} ({scenario['reason']})")
                else:
                    # This is expected for sensitive files
                    self.assertFalse(exists, f"Sensitive file should not be accessible: {scenario['file']}")
                ...
                try:
                    if os.path.exists(temp_file):
                        ...
                        self.assertFalse(os.path.exists(temp_file), 
                                       f"Temporary file not cleaned up: {temp_file}")
                except:
    ...
    def test_04_file_upload_validation(self):
        ...
        upload_scenarios = [
            ...
            {'filename': 'script.php', 'content_type': 'application/x-php', 'size': 512, 'valid': False},
            {'filename': 'malware.exe', 'content_type': 'application/x-executable', 'size': 10240, 'valid': False},
            {'filename': 'huge_image.jpg', 'content_type': 'image/jpeg', 'size': 100*1024*1024, 'valid': False},  # 100MB
            {'filename': '../../../etc/passwd.jpg', 'content_type': 'image/jpeg', 'size': 1024, 'valid': False},
            {'filename': 'image.jpg%00.php', 'content_type': 'image/jpeg', 'size': 1024, 'valid': False}
        ]
        ...
        for scenario in upload_scenarios:
            with self.subTest(filename=scenario['filename']):
                ...
                self.assertEqual(is_valid, scenario['valid'], 
...
class TestAuthenticationSecurity(unittest.TestCase):
    ...
    def test_01_password_policy_validation(self):
        ...
        for scenario in password_scenarios:
            with self.subTest(password=scenario['password']):
                ...
                self.assertEqual(is_valid, scenario['valid'], 
                               f"Password validation failed: {scenario['reason']}")
    ...
    def test_02_session_management(self):
        ...
        session_scenarios = [
            {'session_id': 'abc123', 'timestamp': time.time() - 3600, 'valid': False, 'reason': 'Expired'},
            {'session_id': 'xyz789', 'timestamp': time.time() - 10, 'valid': True, 'reason': 'Valid'},
            {'session_id': '', 'timestamp': time.time(), 'valid': False, 'reason': 'Empty session'},
            {'session_id': None, 'timestamp': time.time(), 'valid': False, 'reason': 'Null session'},
            {'session_id': 'a' * 1000, 'timestamp': time.time(), 'valid': False, 'reason': 'Too long'}
        ]
        ...
        for scenario in session_scenarios:
            with self.subTest(session_id=scenario['session_id']):
                ...
                self.assertEqual(is_valid, scenario['valid'], 
                               f"Session validation failed: {scenario['reason']}")
    ...
    def test_03_access_control_validation(self):
        ...
        for scenario in access_scenarios:
            with self.subTest(role=scenario['role'], action=scenario['action']):
                ...
                self.assertEqual(has_access, scenario['allowed'], 
    ...
    def test_02_data_transmission_security(self):
        ...
        for scenario in transmission_scenarios:
            with self.subTest(protocol=scenario['protocol']):
                ...
                self.assertEqual(is_secure, scenario['secure'], 
        ...
        for log_entry in log_entries:
            with self.subTest(log_entry=log_entry):
                ...
                if has_sensitive:
                    ...
                    self.assertIn('mask', "Sensitive information detected in log - should be masked")
