...
@unittest.skip(
    "Quarantined: targets removed Code/YOLO/darkflow modules (vehicle_detection_modern, simulation)"
)
class TestDetectionSimulationPipeline(unittest.TestCase):
    ...
    def test_02_simulation_receives_detection_data(self):
        ...
        mock_detection_results = {"car": 3, "bike": 1, "bus": 1, "truck": 0, "rickshaw": 2}
                                        ...
                                        with patch("simulation.truckTime", 2.5):
                                            with patch("simulation.rickshawTime", 2.25):
                                                with patch("simulation.noOfLanes", 2):
                                                    greenTime = (
                                                        (mock_detection_results["car"] * 2)
                                                        + (
                                                            mock_detection_results["rickshaw"]
                                                            * 2.25
                                                        )
                                                        + (mock_detection_results["bus"] * 2.5)
                                                        + (mock_detection_results["truck"] * 2.5)
                                                        + (mock_detection_results["bike"] * 1)
                                                    ) / (2 + 1)
...
@unittest.skip("Quarantined: imports removed legacy 'app' module")
